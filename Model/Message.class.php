...
class Message{
    ...
    public static function getLastMessages():Array{
        $request=$GLOBALS['database']->prepare("SELECT * FROM messages WHERE creation_time>DATE_SUB(NOW(),INTERVAL 1 HOUR) ORDER BY id ASC LIMIT 50");
        $request->execute();
    ...
    public static function getLastId():int{
        $request=$GLOBALS['database']->prepare("SELECT id FROM messages ORDER BY id DESC LIMIT 1");
        $request->execute();
        if($result=$request->fetch()) return $result['id'];
        return 0;
    }
    ...
    public static function getNewMessages(string $minId):Array{
        $request=$GLOBALS['database']->prepare("SELECT * FROM messages WHERE id>:minId ORDER BY creation_time ASC");
